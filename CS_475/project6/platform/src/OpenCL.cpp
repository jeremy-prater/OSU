...
bool OpenCL::InitOpenCL()
{
	...
	status = clGetDeviceIDs( platform, CL_DEVICE_TYPE_GPU, 1, &openCL_device, NULL );
	if( status != CL_SUCCESS )
	{
...
{
	status = clEnqueueWriteBuffer(openCL_parent->GetCmdQueue(), buffer, CL_FALSE, 0, bufferSize, source, 0, NULL, NULL );
	if( status != CL_SUCCESS )
	{
...
OpenCLKernel::OpenCLKernel (OpenCLProgram * program, const char * name)
{
	cl_int status;
...
{
	cl_int status = clEnqueueNDRangeKernel(parent->GetCmdQueue(), kernel, 1, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL );
	if( status != CL_SUCCESS )
	{
